...
Package \pkg{resde} computes maximum-likelihood (ML) parameter estimates for reducible stochastic differential equation (RSDE, more on those in a minute). Observations are at discrete points in time, not necesarily evenly spaced, and may include measurement error. Currently, \pkg{resde} handles only univariate time-independent RSDEs. Some types of multivariate models might be added in the future. In the simplest case, there is a single individual or sampling unit (\emph{unit}, for short). Alternatively, there may be a set of units, with some parameters common to all units (\emph{global}), and others specific to each unit (\emph{local}). Estimation in these \emph{two-level hierarchical models} can use either fixed effects methods, or more fashionable mixed effects methods.

RSDEs are SDEs that can be reduced to linear by a change of variables. This may seem unduly restrictive, but we shall see that many important problems can be formulated this way. In particular, linearizing transformations exist for most of the univariate growth models encountered in the literature. Esentially, any differential equation with an invertible closed-form solution can be linearized (section \ref{sec:more}).
...
\end{equation}
with a posibly random initial condition
...
\[  \dd X^c = b(a^c - X^c) \dd t + \sqrt{b} \dd W \;,\]
with initial condition fixed at the origin,
can be specified by the transformation $Y = \varphi(X, c) = X^c$, and the parameter substitutions $\beta_0 \rightarrow b a^c, \beta_1 \rightarrow -b, \mu_p \rightarrow \sqrt{b},
t_0, x_0, \mu_0 \rightarrow 0$. If there are no measurement errors, one would set $\mu_m \rightarrow 0$, otherwise $\mu_m \rightarrow 1$.
...
@
\noindent
Always check the displayed result to see if that is what you meant. The display can be obtained again with \verb|sdemodel_display(exmpl)|. More examples below.
...
\end{equation*}
where the $\epsilon_i$ are normally distributed with mean 0 and variance $\sigma_m^2$.
...
0.5024$, $\sigma_p = 0$, and $\sigma_m = 0.04865$, with a maximized log-likelihood value
of $-4.0$. I'll explain \code{eta} shortly. This would suggest that most of the variability arises from measurement errors. However, $\sigma_p = 0$ is at the boundary of the admissible values, and therefore this could easily be a local optimum different from the global one. To confirm, we force $\sigma_m=0$, as suggested by the warning:
<<>>=
...
@
\noindent
The derivative produced by \code{Deriv::Deriv()} is a little messy, but it works. On esthetic grounds, one might include in \code{sdemodel{}} the simpler equivalent \verb|phiprime = c*x^(c-1)/(x^c - a^c)|.
...
As with any optimization, one should be careful and try different starting values, because local optima can occur. For instance, in the last run, changing the start to \code{a=60} produces a different solution.
...
For hierarchical models, one must indicate a variable that identifies the units in parameter \code{unit} of \code{sdefit()}, \code{"Seed"} in this case. Also, \code{global} and \code{local} are used instead of \code{start}. Starting values for locals can be vectors with one value for each unit, or a single value that applies to all.
...
@
\noindent
Here the AIC or BIC have to be used for the comparison, because the number of free parameters is different. For AIC and BIC smaller is better. They indicate that this is worse than the single-local versions.
...
By the way, if you are mystified by the strangely-looking Box-Cox transformation, eq.~\eqref{eq:bc}, don't be. It is basically a power transformation with a twist, considering that linear (or more precisely affine) transformations are often ``uninteresting''. The point is that the limit $\lim_{c \rightarrow 0} (x^c - 1)/c = \ln x$ makes the transformation continuous at $c=0$, as a function of $c$. In the process including the logarithm as a special case. Hiding those details inside the definition, using $x^{(c)}$ saves us the hassle of having to talk about the special case all the time. And the logarithmic transformation comes along for free. We'll come back to this, with a vengeance, in Section \ref{sec:more}. Physicists discovered the transformation independently, looking from the other end, calling it a \emph{generalized logarithm} denoted by $\ln_c(x)$.
...
The mixed effects method uses \code{nlme()} instead of \code{nls()}. This is chosen by setting \code{method = "nlme"} in \code{sdefit()} (the default is \code{"nls"}).
...
@
\noindent
Convergence fails. There is an optional argument \code{control} in \code{sdefit()}, which acepts a list of control parameters to be passed on to \code{nlme()} or \code{nls()}. Use it to increase the ``PNLS tolerance'' to 0.01, from the default 0.001:
<<>>=
(blocal_mx <- sdefit(m, x="height", t="age", unit="Seed",
              data=Loblolly, global=c(a=70, c=0.5), local=c(b=0.1),
...
\code{Deriv()} seems to do a good job of producing transformation derivatives, although as we have seen, sometimes not in the simplest possible form. If desired, the argument \code{phiprime} can be used. Either way, \code{sdefit()} generates global functions \code{phi()} and \code{phiprime()} to compute transformation and derivatives, respectively. For trouble-shooting, setting \code{trfuns=FALSE} in \code{sdefit()} suppresses the generation of these functions, allowing the user to provide his/her own. If necessary, you can inspect the generated \code{phi()} and \code{phiprime()} to see the proper format. Or run something like \verb|phi <- str2fun_theta("(x/a)^c")| for a starting point.

\subsection{Under the hood}

The aim of \pkg{resde} is to facilitate the application of the function \code{uvector()} from \citet{sdes}. That function uses tricks based on work by Furnival and by Box and Cox, to compute values such that minimizing the sum of their squares produces maximum-likelihood parameter estimates. The sum of squares is minimized with \code{nls()} from package \pkg{stats}, or with \code{nlme()} from package \pkg{nlme}. Setting up a call of \code{uvector()}, as done in \citet{sdes}, can be rather complicated, a process that is ``mechanized'' by \code{sdemodel()} and \code{sdefit()}. The generated call of \code{uvector()} can be seen with \code{formula(f\$fit)} and \code{(f\$fit)\$call}, where \code{f} is the output from \code{sdefit()}. Conceivably, there may be applications where it could be necessary to use \code{uvector()} directly.
...
@
\noindent
We see that there are local optima at $\eta = 0$ ($\sigma_m = 0$), and at $\eta = 1$ ($\sigma_p = 0$). This is what was causing trouble with the optimizations. However, the significance of the log-likelihood differences is marginal. One may conclude that the data cannot tell us much about the error structure, so that we are justified in choosing it from prior knowledge. E.g., if the height observations were derived from tree ring observations, the measurement errors may be negligible compared to the environmental noise, and $\sigma_m = 0$ would be reasonable.
...
A univariate time-invariant (aka automomous) differential equation has the form
\[  \der{x}{t} = f(x) \;. \]
...
\[
    \int\frac{\dd x}{f(x)} = t \;.
\]
Defining the integral as $F^{-1}(x)$ we are back to the situation above. Of course, luck is needed for the integral to have a closed form (analytical solution). On the other hand, it might be interesting to see how well the estimation method works if \code{phi()} and \code{phiprime()} are calculated by numerical integration.
...
\end{equation}
in terms of the Box-Cox transformation defined in eq.~\eqref{eq:bc}. For instance, the Richards is the spacial case $\beta = 0$, and the Hosfeld IV corresponds to $\alpha = -1, \beta < 0$ (\citet{uni}, and \url{http://forestgrowth.unbc.ca/grex/}).
...
\fig{grex}{Sigmoid growth equations determined by the unified transformation with parameters $a = \alpha$ and $b = \beta$. Items in parenthesis correspond to negative scale factors on $x$ and $t$ (reversed axes). Contours indicate the height of the inflection point relative to the range of $x$. From Garc\'ia (2005, 2008), see \url{http://forestgrowth.unbc.ca/grex/}  or \citet{uni} for references.}
...
\begin{sloppypar}
One could also try the reversed form with \code{1-x/a}. The default \code{reverse="auto"} in \code{unitran()} does that automatically, but the discontinuity at $\beta = 0$ causes the optimization to fail. Or perhaps one could experiment with the form corresponding to eq.~\eqref{eq:linear}. There are limits to what is worth doing with 6 data points, but you get the idea.
\end{sloppypar} 
